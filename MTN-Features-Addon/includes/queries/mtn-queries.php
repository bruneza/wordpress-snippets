 ...
 * 'skip_nuthumbnail' = boolean,
 * ]
 * 
 * @return array
 */
function processOutput($query, $conditions = null)
{
    $finalOutput = array();
    while ($query->have_posts()) {
        $post = $query->the_post();

        if (isset($conditions['skip_nothumbnail'])) {
...
function processArgs($additionalArgs)
{
...
function postsRender($settings, $terms = null, $output = null, $conditions = null)
{
    ...
    if ($query->have_posts()) {
        if (isset($output))
            $posts = filterPost(processOutput($query, $conditions), $output);
        else
